...
module SDG.Nilpotent.Base where
  ...
  module Experimental (ℝ@(R , str) : CommRing ℓ) where
    ...
    -- SpecChar : {A : CommAlgebra ℝ ℓ} → (n : ℕ) → (d : zeroLocus {ℓ} {ℝ} {1} 1 (λ _ → gen n) A) → (Σ[ d ∈ ⟨ A ⟩ ] exp-num {A} d n ≡ CommAlgebraStr.0a (snd A))
    -- SpecChar {A} n d = zeroLocus₁ 1 (λ _ → gen n) d zero , (sym (evPolyChar A n (zeroLocus₁ 1 (λ _ → gen n) d))) ∙ (zeroLocus₂ 1 (λ _ → gen n) d zero)
    -- SpecCharInv : {A : CommAlgebra ℝ ℓ} → (n : ℕ) → (Σ[ d ∈ ⟨ A ⟩ ] exp-num {A} d n ≡ CommAlgebraStr.0a (snd A)) → (zeroLocus {ℓ} {ℝ} {1} 1 (λ _ → gen n) A)
    -- SpecCharInv {A} n d = buildInZeroLocus 1 (λ _ → gen n) (λ x → fst d) λ i → evPolyChar A n (λ x → fst d) ∙ (snd d)

    -- SpecLocusIso : {A : CommAlgebra ℝ ℓ} → (n : ℕ) →  Iso (zeroLocus {ℓ} {ℝ} {1} 1 (λ _ → gen n) A) (Σ[ d ∈ ⟨ A ⟩ ] exp-num {A} d n ≡ CommAlgebraStr.0a (snd A))
    -- Iso.fun (SpecLocusIso {A} n) = SpecChar n
    -- Iso.inv (SpecLocusIso {A} n) = SpecCharInv n
    -- Iso.rightInv (SpecLocusIso {A} n) = λ b i → {!   !}
    -- Iso.leftInv (SpecLocusIso {A} n) = λ a → {!   !}
  
    


    -- p2 : ⟨ Polynomials {ℓ} {ℝ} 1 ⟩
    -- p2 = exp-var zero 2
    -- evPolyChar2 : (A : CommAlgebra ℝ ℓ) → (v : ⟨ A ⟩) → evPoly A p2 (λ _ → v) ≡ ((snd A) CommAlgebraStr.· v) (Theory.inducedMap A (λ _ → v) ((Construction.var zero) Construction.· (CommAlgebraStr.1a (snd (Polynomials 1)))))
    -- evPolyChar2 A v = refl

    -- p : (n : ℕ) → ⟨ Polynomials {ℓ} {ℝ} 1 ⟩
    -- p n =  exp-var zero n
    -- evPolyChar : (n : ℕ) → (A : CommAlgebra ℝ ℓ) → (v : ⟨ A ⟩) → evPoly A (p n) (λ _ → v) ≡ exp-num {A} v n
    -- evPolyChar zero A v = evPoly A (p zero) (λ _ → v) ≡⟨ {!   !} ⟩ 
    --                       CommAlgebraStr.1a (snd A) ≡⟨ {!   !} ⟩ 
    --                       CommAlgebraStr.1a (snd A) ≡⟨ refl ⟩ 
    --                       exp-num {A} v zero ∎
    -- evPolyChar (suc n) A v = {!   !}   