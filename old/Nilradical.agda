...
module _ (Ring@(R , str) : CommRing ℓ) where
  ...
  isNilpotent x = ∃[ n ∈ ℕ ] x ^ (ℕsuc n) ≡ 0r

  open RadicalIdeal
  open CommIdeal Ring
  open isCommIdeal
  --Nilradical is Radical Ideal of (0)
  ...
  private
    [_] = [_]/ {ℓ} {Ring} {CommIdeal→Ideal nilradical}

  helper1 : (x : R) (n : ℕ) → (x ^ n) ∈ nilradical → x ∈ nilradical 
  helper1 x n = ^∈√→∈√ Ring 0Ideal x n
...
--                           → isNilpotent y
--                           → isNilpotent (x + y)
--   isNilpotent+ {x = x} {y = y}  = map2 (λ α β → 2 ·ℕ (max (fst α) (fst β)) , lem1 α β)
--     where
--       lem1 : (α : Σ ℕ (λ n → x ^ ℕsuc n ≡ 0r)) → 
--              (β : Σ ℕ (λ m → y ^ ℕsuc m ≡ 0r)) → 
--              (x + y) · (x + y) ^ (2 ·ℕ max (fst α) (fst β)) ≡ 0r -- (x + y) ^ (max (fst α) (fst β) +ℕ (max (fst α) (fst β) +ℕ ℕzero)) ≡ 0r
--       lem1 α β = {!   !}
-- --   {- 0r is the trivial linear Combination -}
--   isNilpotent0 : isNilpotent 0r
--   isNilpotent0 = ∣ 1 , 0LeftAnnihilates (0r ^ 1) ∣
--   isLinearCombination0 V = ∣ _ , sym (dist0 V) ∣
        
  --map2 λ α β → (λ i → α .fst i + β .fst i)
